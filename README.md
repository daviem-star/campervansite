...
- The collapsible desktop left rail provides app-level `Dashboard`, `Today`, `Saved Places`, and `Bookings` destinations, while account and sync controls stay in utility chrome.
...
- Review a desktop Today workspace plus read-only cross-trip Saved Places and Bookings views derived from existing trip stops.
