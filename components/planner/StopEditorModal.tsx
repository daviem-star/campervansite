...
export default function StopEditorModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 px-4 py-8">
      <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-2xl border border-ui-border bg-ui-raised p-5 shadow-2xl">
        <div className="mb-4 flex items-center justify-between">
          <h3 className="text-lg font-semibold text-slate-100">{dialogTitle}</h3>
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg border border-ui-border px-3 py-1 text-sm text-slate-300 transition hover:bg-ui-surface-alt"
          >
            Close
          </button>
        </div>
        ...
        <form onSubmit={onSubmit} className="space-y-4">
                  ...
                  <button
                    ...
                    className={`rounded-full border px-3 py-1.5 text-xs font-semibold transition ${
                      type === option.id
                        ...
                        : "border-ui-border text-slate-300 hover:bg-ui-surface-alt"
                    }`}
                  >
                    {option.label}
                  </button>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className="w-full rounded-xl border border-ui-border bg-ui-surface px-3 py-2 text-sm text-slate-100 focus:border-ui-accent focus:outline-none"
            />
          </div>
          ...
          {type === "ferry" ? (
            <div className="space-y-3 rounded-xl border border-cyan-500/40 bg-cyan-500/10 p-4">
              <div className="grid gap-3 sm:grid-cols-2">
                <StopSearchInput
          ...
          {type === "point_of_interest" ? (
            <div className="space-y-3 rounded-xl border border-orange-500/40 bg-orange-500/10 p-4">
              <StopSearchInput
                label="POI location"
          ...
          <div className="flex items-center justify-end gap-2">
            <button
              type="button"
              onClick={onClose}
              className="rounded-xl border border-ui-border px-4 py-2 text-sm font-semibold text-slate-200 transition hover:bg-ui-surface-alt"
            >
              Cancel
            </button>
            <button
              type="submit"
              disabled={isSaving}
              className="rounded-xl bg-ui-accent-strong px-4 py-2 text-sm font-semibold text-white transition hover:bg-ui-accent-deep disabled:cursor-not-allowed disabled:bg-slate-600"
            >
