...
const AUTO_DOCS_COMMIT_SUBJECT = "docs(status): refresh generated project docs";
...
function filterAutoDocsCommits(entries) {
  return entries.filter((entry) => normalizeSubject(entry.subject) !== AUTO_DOCS_COMMIT_SUBJECT);
}
...
function generateProjectStatus() {
  const allCommits = filterAutoDocsCommits(getCommitEntries());
  const latestCommit = allCommits[0];
  const recentNonMergeCommits = filterAutoDocsCommits(getCommitEntries({ noMerges: true })).slice(0, 8);
  const hotAreaCounts = aggregateAreas(filterAutoDocsCommits(getCommitEntries({ noMerges: true })).slice(0, 10)).slice(0, 6);
  ...
  const generatedAt = latestCommit?.date ?? "unknown";
...
- Total substantive commits on \`HEAD\`: ${allCommits.length}
...
function generateRecentWork() {
  const commits = filterAutoDocsCommits(getCommitEntries({ reverse: true }));
